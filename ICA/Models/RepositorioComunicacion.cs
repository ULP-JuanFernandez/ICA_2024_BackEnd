...
namespace ICA.Models
{
    public class RepositorioComunicacion : RepositorioBase, IRepositorioComunicacion
    {
        ...
        public int Alta(Comunicacion entidad)
        {
            if (entidad == null)
            {
            ...
            const string sql = @"
                                INSERT INTO Comunicacion (Titulo, Creador, Fecha, Sinopsis, Integrantes, Imagen, Video, Estado, GeneroId, EtiquetaId, MateriaId)
            ...
            using (var command = new SqlCommand(sql, connection))
            {
                ...
                command.Parameters.AddWithValue("@sinopsis", entidad.Descripcion);
        ...
        public int Baja(int id)
        {
            ...
            try
            {
                using (var connection = new SqlConnection(connectionString))
                {
                    string sql = @"DELETE FROM Comunicacion WHERE Id = @id";
        ...
        public int Modificacion(Comunicacion entidad)
        {
            if (entidad == null)
            {
                throw new ArgumentNullException(nameof(entidad), "La entidad no puede ser nula.");
            ...
            catch (SqlException ex)
            {
                ...
                throw new ApplicationException("Se produjo un error al intentar modificar el comunicación.", ex);
            }
            catch (Exception ex)
            {
        ...
        {
            var peliculas = new List<Comunicacion>();
        ...
        public Comunicacion ObtenerPorId(int id)
        {
            ...
            string query = @"SELECT c.Id, c.Titulo, c.Descripcion, c.Integrantes, c.Creador, c.Fecha, c.Estado, c.Video, c.GeneroId,c.EtiquetaId, c.MateriaId,
                            g.Nombre AS GeneroNombre, m.Nombre AS MateriaNombre, e.Nombre AS EtiquetaNombre
                     FROM Comunicacion c 
                     INNER JOIN Genero g ON g.Id = c.GeneroId
                     INNER JOIN Materia m ON m.Id = c.MateriaId
